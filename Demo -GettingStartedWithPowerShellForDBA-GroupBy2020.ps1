...
Get-Command -Module dbatools # Returns all cmdlets in the module

(Get-Command -Module dbatools).count # To find out the count of cmdlets available in the SqlServer module

Get-Help Get-DbaDatabase -online # Get info on cmdlet Get-Service directly from MS Docs. This will open a browser.
...
$SQLSrvc | Select-Object DisplayName, ServiceName, Status # This is an example how to display multiple properties.
...
$passwd = Read-Host "Please enter password for $username" -assecurestring 
...
Get-SqlInstance -ServerInstance "WIN2019-DC1" -Credential $cred 
...
Backup-DbaDatabase -SqlInstance $Inst -Database $db -Path $bkpfile -CopyOnly -CompressBackup -Verify
...
Copy-DbaDatabase -Source "SQL2019-01" -SourceSqlCredential $cred -Database "AdventureWorksLT2019" -Destination "SQL2019-02" -DestinationSqlCredential $cred -UseLastBackup
...
# Return all cmdlets with backup in their name
get-command -Module DBATools | ? name -Like "*reg*"
...
New-DbaLogin -SqlInstance "SQL2019-02" -Login "Milo" -enable -DefaultDatabase master | Set-DbaLogin -AddRole sysadmin -Enable

# Checking roles assigned to a login
(Get-DbaLogin -SqlInstance "sql2019-01" -Login "Milo").listmembers() 

# Create user in DB for login Milo2
New-DbaDbUser -SqlInstance "SQL2019-01" -Database "AdventureWorksLT2019" -login "Milo" -Username "Milo" 

# How do we assign roles in the database for the new user
$user = Get-DbaDbUser -SqlInstance "SQL2019-01" -Database "AdventureWorksLT2019" | ? Name -EQ "Milo"
...
Get-help Repair-DbaDbOrphanUser -FULL -SqlInstance "sql2019-01" -DatabaseName "AdventureWorksLT2019" -remove